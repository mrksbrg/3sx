...
| `opening_bg0.c` | 8.12 | a family plateau, measured twice, and the score still holds. Both Complex Method moves - extracting `op_bg0_0003`'s inner switch, and splitting `op_bg0_lay_blocks_6` - make a seventh `op_bg0_lay_blocks_*` member and measure **8.12 -> 8.03**. **Moving** the `0x4B` arm up the chain instead creates no new function and costs the same 0.09, because that arm was the only thing making `_6` unlike `_4`. What did pay is Recipe C on the seven-statement run `op_bg0_0010` and `op_bg0_0012` share: flat on score, and `op_bg0_0010` leaves the duplication group |
